...
These are the **marks'** colours, and the artwork carries them itself. The Kaia landing
page has its own, slightly warmer set of CSS variables next to them (`--ink #241c15`,
`--sand #ede6d6`, `--gold #e3a857` in `resources/css/kaia-home.css`) — do not assume
`var(--sand)` is the sand in this table. Where a mark sits on that page it keeps its own
fills; only the wordmark's letters follow the page, because they take `currentColor`.
...
- **Outfit SemiBold** is used for the Kaia wordmark **only**, and it ships as
  *outlined vector artwork, never as a webfont*. Adding a second family to the build
  ...
  it places the artwork. Outfit is licensed under the SIL Open Font License; outlines
  drawn from it in a logo are a permitted use, and no font file is redistributed here.
...
Generated by `resources/brand/build_marks.py` — it pulls the two letters out of Outfit
SemiBold as outlines, drops the i's round dot, and puts the diamond in its place. Run it
again only if the geometry has to change; the files it writes are the artwork.

In `public/images/kaia/`:

| File | What it is |
| --- | --- |
| `kaia-wordmark.svg` | letters in `currentColor`, diamond in tan — the default |
| `kaia-wordmark-mono.svg` | the whole mark in one ink |
| `kaia-avatar-light.svg` | brown circle, tan letter — for light surfaces |
| `kaia-avatar-dark.svg` | sand circle, brown letter — for dark surfaces |
| `kaia-icon-tile.svg` | the app-icon tile |
| `kaia-k.svg` | the bare letter in `currentColor` |

Two more places carry copies on purpose:

- `resources/js/components/KaiaWordmark.vue` and `KaiaAvatar.vue` inline the same paths,
  so the marks take their colour from CSS and cost no extra request.
- `resources/images/kaia-wordmark.png` is a raster copy for the trip-plan PDF: dompdf
  cannot render the flipped transform the outlined SVG is built on. Regenerate it beside
  the SVGs whenever the wordmark changes.

## Where the mark is in the product

- **Above the conversation** (`HeroChat.vue` → `.chat-head`): avatar, wordmark and one
  line saying who she is. Deliberately not an avatar beside every message — on a phone
  that gutter comes out of the width the conversation needs to read in.
- **On the plan** (`ItinerarySection.vue` → `.planned-with`): "planned with kaia" under
  the heading, as a byline. A shared plan is often the first thing a second traveller
  sees of her.
- **In the trip-plan PDF** (`pdf/trip-plan.blade.php` footer), the same byline.

**Not in the mail templates.** Every email the platform sends comes from NamibWay — a
request to a lodge, a confirmation, an invoice — and none of them is Kaia's work. If a
mail ever carries a plan, it gets the byline then.

## Status

Settled and built: the wordmark, the avatar, both colour cases, the artwork, and the
three places above.

Still open:

- **The endorsement lockup** ("kaia │ a travel companion by NamibWay") exists as a design
  but not as a file; it is a composition of the wordmark, a rule and a line of text, so
  it belongs in whatever surface needs it rather than in a single SVG.
- **The figurative trademark filing** — the artwork is final now, so this is unblocked.
- **The app icons and splash screens** still derive from the NamibWay compass, which is
  correct: those are the product's icons, not Kaia's. `kaia-icon-tile.svg` exists for the
  day Kaia gets a surface of her own.
