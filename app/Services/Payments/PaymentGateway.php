...
class PaymentGateway
{
     ...
     *
     * When the provider declares NAD in its `supportedCurrencies()`, a NAD
     * folio passes straight through with no conversion. For any other provider
     * that does not, the NAD↔ZAR peg in `config/payments.currency_pegs` is
     * applied — NAD is pegged 1:1 to ZAR under the Common Monetary Area. The
     * rate is stored on the intent so a later refund returns the same money,
     * and so a peg that ends does not rewrite what was charged.
     *
