...
/**
 * The fields that describe one room/unit type, defined once for the two places
 * they are edited: the "Room types" tab of the admin listing form (a
 * relationship Repeater) and BookableUnitsRelationManager's modal in the
 * partner panel.
 *
 * One definition, because the two surfaces edit the same rows — a lodge
 * entering a rate in its own panel and a content manager checking it in /admin
 * have to be looking at the same form, or one of them is working from a field
 * the other cannot see.
 */
class BookableUnitSchema
{
    /**
    ...
    public static function schema(): array
    {
        return [
            ...
            Forms\Components\Placeholder::make('gallery_preview')
                ...
                // Not typed to BookableUnit: the same schema is used inside a
                // Repeater, where the injected record is whatever the item's
                // container carries — the parent Listing for a row that has
                // not been saved yet. Anything that is not a unit simply has
                // no photos to preview.
                ->content(function (?Model $record): HtmlString {
                    $images = $record instanceof BookableUnit ? ($record->gallery ?? []) : [];
