...
/**
 * Mirrors DestinationSeeder — see that class for the authoritative content
 * and rationale for each destination. Needed as its own migration (not just
 * the seeder) for the same reason 2026_08_02_180100_create_regions_table.php
 * seeds the 14 political regions directly: deploy.sh only ever runs
 * `migrate --force`, never `db:seed`, so a seeder-only destination never
 * reaches production. Written with updateOrCreate-style upserts (matching
 * DestinationSeeder) so it stays safe to re-run by hand if ever needed.
 */
return new class extends Migration
{
    ...
    {
        Destination::whereIn('slug', array_map(
            fn (string $name) => Str::slug($name),
            ['Windhoek', 'Etosha', 'Sossusvlei', 'Swakopmund', 'Spitzkoppe', 'Sandwich Harbour',
                'Skeleton Coast', 'Twyfelfontein', 'Waterberg', 'Fish River Canyon', 'Lüderitzbucht', 'Kolmanskop']
        ))->delete();
    }
};
